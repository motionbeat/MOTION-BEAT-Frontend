...
export const SecondScore = ({ gameData, railRefs, myPosition }) => {
  ...
  useEffect(() => {
    ...
    console.log("유즈이펙트3: 소켓 전송", combo);
    ...
    const scoreUpdateEvents = gameData.players.map((player, index) => {
      ...
      socket.on(eventName, (scoreData, combo, instrument, motionType) => {
        ...
        if (
          ...
          console.log("이벤트수신정보 나의 위치:", myPosition, "송신자 위치:", index);
          if (index !== myPosition) {
...
export const TriggerHitEffect = (target, elem) => {
  ...
  if (closestNote) {
    ...
    console.log("[SL] All Trigger에서 클로젯 노트 Remove: ", closestNote, closestNote.getAttribute("data-index"));
  }
